...
class Time_Windows():
    ...
    def lookup_window(self, timepoint):
        return torch.ones_like(timepoint) * self.window_starts[0], torch.ones_like(timepoint) * self.window_ends[0]
        # if timepoint.dim() == 0:
...
# Modified from https://github1s.com/magic-research/piecewise-rectified-flow/blob/main/src/scheduler_perflow.py#L125-L370
class PeRFlowScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        num_time_windows = 1,  ### Modified to make it flow Straight!
    ):
        if trained_betas is not None:
            self.betas = torch.tensor(trained_betas, dtype=torch.float32)
        elif beta_schedule == "linear":
            self.betas = torch.linspace(beta_start, beta_end, num_train_timesteps, dtype=torch.float32)
        elif beta_schedule == "scaled_linear": # Inside this!
        ...
        self.time_windows = Time_Windows(num_windows=num_time_windows, 
    ...
    def training_losses(self, model_output, orig_sample, noised_sample, timepoints):
        if self.config.prediction_type == "ddim_eps":
            with torch.no_grad():
                _, _, _, _, _, alphas_cumprod_start, alphas_cumprod_end = self.get_window_alpha(timepoints)
                alphas_cumprod_start = alphas_cumprod_start[:,None,None,None].to(device=model_output.device)
                alphas_cumprod_end = alphas_cumprod_end[:,None,None,None].to(device=model_output.device)
                lambda_s = (alphas_cumprod_end / alphas_cumprod_start)**0.5
                eta_s = (1-alphas_cumprod_end)**0.5 - ( alphas_cumprod_end / alphas_cumprod_start * (1-alphas_cumprod_start) )**0.5
                targets = (orig_sample - lambda_s * noised_sample) / eta_s
